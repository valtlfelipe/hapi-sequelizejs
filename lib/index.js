    ...
    return opts.sequelize.authenticate().then(() => {
        ...
        if (opts.models) {
            ...
            return Models.getFiles(opts.models).then((files) => {
                return Models.load(files, opts.sequelize.import
                        .bind(opts.sequelize))
                    .then((models) => Models.applyRelations(
                        models))
                    .then((models) => {
...
exports.register = function (server, options, next) {
    ...
    const getDb = (request) => {
        return function getDb(name) {
            if (!name) {
                const key = Object.keys(request.server.plugins[Pkg.name])
                    .shift();
                return request.server.plugins[Pkg.name][key];
            } else if (!request.server.plugins[Pkg.name].hasOwnProperty(
                    name)) {
                throw new Error(
